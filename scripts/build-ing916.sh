#!/usr/bin/env bash
# ============================================================
# 编译 ing916 + 自动更新 compile_commands.json（容器内运行）
# 用法（容器内）：
#   bash /home/ohos/openharmony/build-ing916.sh [hb build 参数...]
# 宿主一键（wrapper）：bash /home/ming/openharmony/build-ing916.sh
# ============================================================
set -euo pipefail
...
echo "==== [1/4] 同步 SDK 符号表与 ld 布局 (apis.json/meta.json -> symdefs.g/ing916.ld) ===="
python3 device/soc/ingchips/scripts/gen_symdefs.py ING9168xx
python3 device/soc/ingchips/scripts/gen_ld.py ING9168xx

echo "==== [2/4] hb set -p ing916 + hb build ===="
hb set -p ing916 2>&1 | tail -1
hb build "$@"

echo "==== [4/4] 更新 compile_commands.json ===="
"$NINJA" -C out/ing916/ing916 -t compdb cxx cc > /tmp/cc_raw.json
